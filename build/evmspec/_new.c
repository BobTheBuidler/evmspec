...
PyInit__new(void)
{
    ...
    tmp = PyImport_ImportModuleLevel("evmspec__mypyc", NULL, NULL, fromlist, 0);
    if (!tmp) return NULL;
    ...
    if (deps_capsule != NULL) {
        int (*deps_func)(void) = (int (*)(void))PyCapsule_GetPointer(
            deps_capsule, "evmspec__mypyc.ensure_deps");
    ...
    void *init_func = PyCapsule_GetPointer(capsule, "evmspec__mypyc.init_evmspec____new");
    Py_DECREF(capsule);
    if (!init_func) {
        return NULL;
    }
